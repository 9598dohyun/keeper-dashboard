 ...
 * 결제건의 첫상담 / 재상담 분해.
 *
 * 기준 (2026-09-03 변경): **유입일과 결제일이 같으면** 첫상담, 그 밖은 재상담.
 * 합은 결제 건수와 같다.
 *
 * 종전에는 `첫응대시각`까지 결제일과 같아야 첫상담으로 봤는데, 이 필드가 상담 시점이
 * 아니라 **결제 처리 시 소급 입력**되는 경우가 많아(결제건 167건 중 27건이 결제시각과
 * 5분 이내, 메모에는 실제 통화일이 유입일보다 앞선 사례도 있음) 판정에서 뺐다.
 * 8/11 이전 유입분은 아예 값이 없어 무조건 재상담으로 떨어지는 문제도 함께 사라진다.
 *
 * 한계: 상담 횟수를 센 것이 아니라 **유입에서 결제까지 날이 넘어갔는지**를 보는 것이다.
 * 에어테이블에 상담 횟수 필드가 없고, 접촉이력은 리드당 마지막 접촉일 1개만 남아
 * (덮어쓰기 필드) 횟수를 소급 계산할 수 없다. 같은 날 여러 번 상담해 결제된 건도
 * 첫상담으로 잡힌다.
 */
export interface 상담차수Stat {
  ...
  첫상담_pct: number;
  재상담_pct: number;
  /** 재상담 건의 유입→결제 소요일 중앙값. 재상담이 0이면 null */
  재상담_소요일_중앙: number | null;
  /** 재상담 건의 유입→결제 소요일 최대. 재상담이 0이면 null */
  재상담_소요일_최대: number | null;
  /**
